...
function card(agent, index) {
  ...
  return `<article class="weekly-item"><div class="weekly-rank">#${index + 1}</div><div><span class="analysis-label">${escapeHtml(agent.category || category(agent))}</span><h2><a href="/agent/${slug}">${escapeHtml(agent.name)}</a></h2><p>${escapeHtml(agent.tagline || agent.description)}</p><div class="weekly-metrics"><span>Radar ${score.total}</span><span>★ ${Number(agent.stars ?? agent.votes ?? 0).toLocaleString()} stars</span><span>${Number(agent.starDelta ?? agent.voteDelta ?? 0) > 0 ? `+${Number(agent.starDelta ?? agent.voteDelta)} since last scan` : "Newly tracked signal"}</span></div></div></article>`;
}
...
function archiveNavigation(keys, current) {
  const links = keys
    ...
    .map(
      (key) =>
        `<a href="/weekly/${key}"${key === current ? ' aria-current="page"' : ""}>${escapeHtml(key)}</a>`,
    )
    .join("");
  return links
    ? `<nav class="report-archive" aria-label="Weekly report archive"><span>Report archive</span>${links}</nav>`
    : "";
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const [payload, storedReports] = await Promise.all([
      kv.get("agents:latest"),
      kv.get("weekly:reports"),
    ]);
    const reports =
      storedReports &&
      typeof storedReports === "object" &&
    ...
    const demandSignals = ranked.flatMap((agent) =>
      cleanIssueEvidence(agent.evidenceIssues || []).map((issue) => ({
        agent,
        issue,
        theme: opportunityTheme(issue).name,
      })),
    );
    const demandThemes = [
      ...demandSignals.reduce(
        (counts, item) =>
          counts.set(item.theme, (counts.get(item.theme) || 0) + 1),
        new Map(),
      ),
    ].sort((a, b) => b[1] - a[1]);
    const themeMarkup = demandThemes
      .slice(0, 4)
      .map(
        ([theme, count]) =>
          `<a href="/opportunities?theme=${encodeURIComponent(theme)}">${escapeHtml(theme)} <strong>${count}</strong></a>`,
      )
      .join("");
