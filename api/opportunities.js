...
function opportunityCard(item, index) {
  ...
  return `<article class="opportunity-card"><div class="opportunity-rank">#${index + 1}</div><div class="opportunity-content"><div class="opportunity-kicker"><span class="analysis-label">${escapeHtml(item.theme)} · ${escapeHtml(agent.category || category(agent))}</span><strong>Evidence ${item.score}/100</strong></div><h2><a href="/opportunity/${encodeURIComponent(issue.id)}">${escapeHtml(issue.title)}</a></h2><p class="opportunity-project">Observed in <a href="/agent/${slug}">${escapeHtml(agent.name)}</a> · ${escapeHtml(agent.language || 'Unknown')} · ${escapeHtml(agent.license || 'License not declared')}</p>${excerpt}<div class="opportunity-metrics"><span>${Number(issue.comments || 0)} comments</span><span>${Number(issue.reactions || 0)} positive reactions</span><span>${age ? `${age} days open` : 'Open duration unknown'}</span><span>Project Radar ${Number(agent.score?.total || scoreBreakdown(agent).total)}</span></div>${labels ? `<div class="opportunity-labels">${labels}</div>` : ''}<div class="opportunity-actions"><a href="/opportunity/${encodeURIComponent(issue.id)}#validation-start">Start guided sprint →</a><a href="${safeUrl(issue.url)}" target="_blank" rel="noopener noreferrer">Original GitHub Issue ↗</a><a href="/agent/${slug}">Project analysis →</a></div></div></article>`;
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const filtered = opportunities.filter((item) => {
      const searchable = `${item.issue.title || ''} ${item.issue.excerpt || ''} ${(item.issue.labels || []).join(' ')} ${item.agent.name || ''} ${item.agent.category || category(item.agent)} ${item.theme}`.toLowerCase();
      const domain = item.agent.category || category(item.agent);
      return (!newOnly || item.isNew) && (activeTheme === 'All' || item.theme === activeTheme) && (activeDomain === 'All' || domain === activeDomain) && queryTerms.every((term) => searchable.includes(term));
    });
    function filterHref({ theme = activeTheme, domain = activeDomain, view = newOnly ? 'new' : '', q = query } = {}) {
    ...
    const filteredView = Boolean(query || activeTheme !== 'All' || activeDomain !== 'All' || newOnly);
