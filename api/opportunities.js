...
function opportunityCard(item, index) {
  ...
  const confidence = item.confidence || evidenceConfidence(issue, item.repositoryCount);
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    opportunities.forEach((item) => { item.confidence = evidenceConfidence(item.issue, item.repositoryCount); });
    ...
    const confidenceCounts = opportunities.reduce((counts, item) => counts.set(item.confidence.level, (counts.get(item.confidence.level) || 0) + 1), new Map());
    ...
    const requestedPattern = String(req.query.pattern || 'All');
    const activePattern = requestedPattern === 'All' || patternCounts.has(requestedPattern) ? requestedPattern : 'All';
    const requestedDomain = String(req.query.domain || 'All');
    const activeDomain = requestedDomain === 'All' || domainCounts.has(requestedDomain) ? requestedDomain : 'All';
    const requestedConfidence = String(req.query.confidence || 'all').toLowerCase();
    const activeConfidence = requestedConfidence === 'all' || confidenceCounts.has(requestedConfidence) ? requestedConfidence : 'all';
    ...
    const filtered = opportunities.filter((item) => {
      const searchable = `${item.issue.title || ''} ${item.issue.excerpt || ''} ${(item.issue.labels || []).join(' ')} ${item.agent.name || ''} ${item.agent.category || category(item.agent)} ${item.theme} ${item.pattern}`.toLowerCase();
      const domain = item.agent.category || category(item.agent);
      return (!newOnly || item.isNew) && (activeTheme === 'All' || item.theme === activeTheme) && (activePattern === 'All' || item.pattern === activePattern) && (activeDomain === 'All' || domain === activeDomain) && (activeConfidence === 'all' || item.confidence.level === activeConfidence) && queryTerms.every((term) => searchable.includes(term));
    });
    function filterHref({ theme = activeTheme, pattern = activePattern, domain = activeDomain, confidence = activeConfidence, view = newOnly ? 'new' : '', q = query } = {}) {
      ...
      if (q) params.set('q', q);
      return escapeHtml(`/opportunities${params.size ? `?${params}` : ''}`);
    }
    const themeLinks = [`<a href="${filterHref({ theme: 'All' })}"${activeTheme === 'All' && !newOnly ? ' aria-current="page"' : ''}>All themes <span>${opportunities.length}</span></a>`, `<a href="${filterHref({ view: newOnly ? '' : 'new' })}"${newOnly ? ' aria-current="page"' : ''}>New in 48h <span>${newCount}</span></a>`, ...[...themeCounts.entries()].sort((a, b) => b[1] - a[1]).map(([theme, count]) => `<a href="${filterHref({ theme, view: '' })}"${!newOnly && activeTheme === theme ? ' aria-current="page"' : ''}>${escapeHtml(theme)} <span>${count}</span></a>`)].join('');
    const domainLinks = [`<a href="${filterHref({ domain: 'All' })}"${activeDomain === 'All' ? ' aria-current="page"' : ''}>All fields</a>`, ...[...domainCounts.entries()].sort((a, b) => b[1] - a[1]).map(([domain, count]) => `<a href="${filterHref({ domain })}"${activeDomain === domain ? ' aria-current="page"' : ''}>${escapeHtml(domain)} <span>${count}</span></a>`)].join('');
    const confidenceNames = { repeated: 'Repeated across projects', supported: 'Supported single Issues', early: 'Early interview leads' };
    const confidenceLinks = [`<a href="${filterHref({ confidence: 'all' })}"${activeConfidence === 'all' ? ' aria-current="page"' : ''}>All confidence levels <span>${opportunities.length}</span></a>`, ...['repeated', 'supported', 'early'].filter((level) => confidenceCounts.has(level)).map((level) => `<a href="${filterHref({ confidence: level })}"${activeConfidence === level ? ' aria-current="page"' : ''}>${confidenceNames[level]} <span>${confidenceCounts.get(level)}</span></a>`)].join('');
    const updatedAt = latest?.updatedAt || new Date().toISOString();
    const canonical = `${SITE_URL}/opportunities`;
    const filteredView = Boolean(query || activeTheme !== 'All' || activePattern !== 'All' || activeDomain !== 'All' || activeConfidence !== 'all' || newOnly);
