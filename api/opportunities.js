...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const filtered = opportunities.filter((item) => {
      ...
      return (!newOnly || item.isNew) && (activeTheme === 'All' || item.theme === activeTheme) && queryTerms.every((term) => searchable.includes(term));
    });
    ...
    const searchForm = `<form class="opportunity-search" action="/opportunities" method="get" role="search"><label for="opportunity-search">Search demand evidence</label><div><input id="opportunity-search" name="q" type="search" maxlength="80" value="${escapeHtml(query)}" placeholder="Try: memory, approval, local model, export…"><button class="button button-secondary" type="submit">Search</button></div>${activeTheme !== 'All' ? `<input type="hidden" name="theme" value="${escapeHtml(activeTheme)}">` : ''}${newOnly ? '<input type="hidden" name="view" value="new">' : ''}</form>`;
    const coverageMarkup = `<section class="coverage-status"><div><span class="analysis-label">ISSUE SCAN COVERAGE</span><strong>${coveredRepositories} / ${coverageAgents.length || 0} repositories · ${newCount} new</strong></div><p>“New” means first detected by the Radar within 48 hours. Issue evidence is collected on a rotating six-hour schedule, and a repository can be fully scanned even when no qualifying signal is found.</p></section>${searchForm}`;
