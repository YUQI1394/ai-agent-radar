...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const filtered = opportunities.filter((item) => {
      ...
      const domain = item.agent.category || category(item.agent);
      return (!newOnly || item.isNew) && (activeTheme === 'All' || item.theme === activeTheme) && (activePattern === 'All' || item.pattern === activePattern) && (activeDomain === 'All' || domain === activeDomain) && queryTerms.every((term) => searchable.includes(term));
    });
    function filterHref({ theme = activeTheme, pattern = activePattern, domain = activeDomain, view = newOnly ? 'new' : '', q = query } = {}) {
    ...
    const filteredView = Boolean(query || activeTheme !== 'All' || activePattern !== 'All' || activeDomain !== 'All' || newOnly);
