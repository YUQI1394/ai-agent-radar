...
function opportunityCard(item, index) {
  ...
  return `<article class="opportunity-card"><div class="opportunity-rank">#${index + 1}</div><div class="opportunity-content"><div class="opportunity-kicker"><span class="analysis-label">${escapeHtml(item.theme)} · ${escapeHtml(agent.category || category(agent))}</span><strong>Evidence ${item.score}/100</strong></div><h2><a href="/opportunity/${encodeURIComponent(issue.id)}">${escapeHtml(issue.title)}</a></h2><p class="opportunity-project">Observed in <a href="/agent/${slug}">${escapeHtml(agent.name)}</a> · ${escapeHtml(agent.language || 'Unknown')} · ${escapeHtml(agent.license || 'License not declared')}</p><div class="opportunity-metrics"><span>${Number(issue.comments || 0)} comments</span><span>${Number(issue.reactions || 0)} positive reactions</span><span>${age ? `${age} days open` : 'Open duration unknown'}</span><span>Project Radar ${Number(agent.score?.total || scoreBreakdown(agent).total)}</span></div>${labels ? `<div class="opportunity-labels">${labels}</div>` : ''}<div class="opportunity-actions"><a href="/opportunity/${encodeURIComponent(issue.id)}">Open validation brief →</a><a href="${safeUrl(issue.url)}" target="_blank" rel="noopener noreferrer">Original GitHub Issue ↗</a><a href="/agent/${slug}">Project analysis →</a></div></div></article>`;
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    agentsById.forEach((agent) => (agent.evidenceIssues || []).forEach((issue) => {
      ...
      seen.add(key);
      opportunities.push({ issue, agent, theme: opportunityTheme(issue).name, score: evidenceScore(issue, agent) });
    }));
    ...
    const filtered = activeTheme === 'All' ? opportunities : opportunities.filter((item) => item.theme === activeTheme);
    const themeLinks = [`<a href="/opportunities"${activeTheme === 'All' ? ' aria-current="page"' : ''}>All <span>${opportunities.length}</span></a>`, ...[...themeCounts.entries()].sort((a, b) => b[1] - a[1]).map(([theme, count]) => `<a href="/opportunities?theme=${encodeURIComponent(theme)}"${activeTheme === theme ? ' aria-current="page"' : ''}>${escapeHtml(theme)} <span>${count}</span></a>`)].join('');
    ...
    const coverageMarkup = `<section class="coverage-status"><div><span class="analysis-label">ISSUE SCAN COVERAGE</span><strong>${coveredRepositories} / ${coverageAgents.length || 0} repositories</strong></div><p>Issue evidence is collected on a rotating schedule. Coverage grows with each six-hour refresh, and a repository can be fully scanned even when no qualifying demand signal is found.</p></section>`;
    ...
    return res.status(200).send(html.replace('</section><section class="opportunity-disclaimer">', `</section>${coverageMarkup}<section class="opportunity-disclaimer">`).replace('<a href="/weekly">Weekly Radar</a>', '<a href="/patterns">Patterns</a><a href="/workspace">Workspace</a><a href="/weekly">Weekly Radar</a>'));
  } catch (error) {
