...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const coverageMarkup = `<section class="coverage-status"><div><span class="analysis-label">ISSUE SCAN COVERAGE</span><strong>${coveredRepositories} / ${coverageAgents.length || 0} repositories · ${newCount} new</strong></div><p>“New” means first detected by the Radar within 48 hours. Issue evidence is collected on a rotating six-hour schedule, and a repository can be fully scanned even when no qualifying signal is found.</p></section>${searchForm}<div class="filter-group-label">Professional field</div><nav class="opportunity-domains" aria-label="Filter opportunities by professional field">${domainLinks}</nav><div class="filter-group-label">Problem theme</div>`;
