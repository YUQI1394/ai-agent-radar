...
const THEMES = [
  { name: 'Integrations', pattern: /integrat|connector|plugin|api|mcp|webhook|provider/i },
  { name: 'Reliability', pattern: /error|fail|retry|timeout|crash|recover|reliab|stability/i },
  { name: 'Developer Experience', pattern: /document|example|install|setup|config|debug|typescript|sdk|cli/i },
  { name: 'Memory & Data', pattern: /memory|context|database|storage|vector|retriev|knowledge|rag/i },
  { name: 'Agent Control', pattern: /workflow|multi-agent|human|approval|observ|trace|monitor|schedule/i }
];

function opportunityTheme(issue) {
  const text = `${issue.title || ''} ${(issue.labels || []).join(' ')}`;
  return THEMES.find((theme) => theme.pattern.test(text))?.name || 'Product Capability';
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    agentsById.forEach((agent) => (agent.evidenceIssues || []).forEach((issue) => {
      ...
      seen.add(key);
      opportunities.push({ issue, agent, theme: opportunityTheme(issue), score: evidenceScore(issue, agent) });
    }));
    ...
    return res.status(200).send(html.replace('<a href="/weekly">Weekly Radar</a>', '<a href="/workspace">Workspace</a><a href="/weekly">Weekly Radar</a>'));
  } catch (error) {
