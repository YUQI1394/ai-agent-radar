...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    agentsById.forEach((agent) => (agent.evidenceIssues || []).forEach((issue) => {
      ...
      seen.add(key);
      const group = groups.get(opportunityTheme(issue).name);
    ...
    const patterns = [...groups.values()].filter((group) => group.issues.length).map((group) => {
      group.score = group.repositories.size * 20 + group.issues.length * 8 + Math.min(30, group.comments * 2 + group.reactions * 3);
      group.issues.sort((a, b) => (Number(b.issue.comments || 0) + Number(b.issue.reactions || 0)) - (Number(a.issue.comments || 0) + Number(a.issue.reactions || 0)));
      return group;
    }).sort((a, b) => b.score - a.score);
    const recurring = patterns.filter((pattern) => pattern.repositories.size >= 2).length;
    const canonical = `${SITE_URL}/patterns`;
    const html = `<!doctype html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="description" content="Recurring product-demand patterns synthesized from traceable GitHub Issues across open-source AI agent projects."><meta name="robots" content="index, follow"><meta property="og:title" content="Pattern Radar · AI Agent Radar"><meta property="og:description" content="See which AI agent problems repeat across independent open-source projects."><meta property="og:url" content="${canonical}"><meta property="og:image" content="${SITE_URL}/og-image.png"><link rel="canonical" href="${canonical}"><link rel="stylesheet" href="/styles.css"><link rel="icon" href="/favicon.svg"><title>Pattern Radar · AI Agent Radar</title></head><body><header class="site-header"><a class="brand" href="/">AI Agent Radar</a><nav class="site-nav"><a href="/">Home</a><a href="/opportunities">Opportunities</a><a href="/patterns" aria-current="page">Patterns</a><a href="/workspace">Workspace</a><a href="/weekly">Weekly Radar</a><a href="/about">About</a></nav></header><main class="page-shell report-shell"><section class="report-hero"><span class="eyebrow">CROSS-REPOSITORY PROBLEM INTELLIGENCE</span><h1>Pattern <span>Radar</span></h1><p>We group qualified Issues by the underlying problem. Signals found across independent repositories carry more weight than isolated feature requests.</p></section><section class="pattern-summary"><div><strong>${patterns.length}</strong><span>Problem themes</span></div><div><strong>${recurring}</strong><span>Cross-project patterns</span></div><div><strong>${seen.size}</strong><span>Traceable Issues</span></div></section><section class="pattern-disclaimer"><strong>Repetition increases confidence, not certainty.</strong><p>Even cross-project evidence still requires direct user interviews and behavioral validation before building.</p></section><section class="pattern-list">${patterns.map(patternCard).join('') || '<div class="empty-state"><h2>No patterns yet</h2><p>Patterns will emerge as the Issue scan covers more repositories.</p></div>'}</section><section class="method-card report-method"><h2>Pattern confidence</h2><p>“Emerging” means one repository. “Cross-project” means two independent repositories. “Recurring” means three or more. Ranking also considers Issue count, comments and positive reactions. Maintenance noise is excluded upstream.</p></section></main><footer class="site-footer"><p>AI Agent Radar · Free, independent open-source intelligence</p><nav class="footer-links"><a href="/opportunities">Opportunities</a><span>·</span><a href="/workspace">Workspace</a><span>·</span><a href="/about">About</a></nav></footer></body></html>`;
