...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const demandSources = new Set(demand.map((item) => String(item.agent.name))).size;
    const demandConfidence = demandSources >= 2 ? 'MULTI-PROJECT EVIDENCE' : 'EARLY SINGLE-PROJECT EVIDENCE';
    ...
    const demandMarkup = `<section class="domain-demand"><div class="domain-demand-heading"><div><span class="analysis-label">LIVE GITHUB DEMAND · ${demandConfidence}</span><h2>${demand.length} qualified signals · ${patternCounts.size} specific problems</h2><p>Evidence currently comes from ${demandSources} independent project${demandSources === 1 ? '' : 's'}. ${demandSources < 2 ? 'Treat this as an interview lead, not a domain-wide trend.' : 'Independent sources reduce the risk of mistaking one project backlog for a broader need.'}</p></div><a class="button button-secondary" href="/opportunities?domain=${encodeURIComponent(domain.name)}">Browse all ${escapeHtml(domain.name)} evidence</a></div><nav class="opportunity-domains" aria-label="Top ${escapeHtml(domain.name)} demand patterns">${patternLinks}</nav><div class="domain-demand-list">${demandItems || '<p>No qualified demand evidence is available yet.</p>'}</div></section>`;
    ...
    const html = `<!doctype html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="description" content="${escapeHtml(domain.description)} Ranked from transparent GitHub evidence."><meta name="robots" content="index, follow"><meta property="og:title" content="Best Open-Source ${escapeHtml(domain.name)} AI Agents"><meta property="og:description" content="${escapeHtml(domain.description)}"><meta property="og:url" content="${canonical}"><meta property="og:image" content="${SITE_URL}/og-image.png"><link rel="canonical" href="${canonical}"><link rel="alternate" type="application/rss+xml" title="AI Agent Radar RSS Feed" href="${SITE_URL}/feed.xml"><link rel="stylesheet" href="/styles.css"><link rel="icon" href="/favicon.svg"><title>Open-Source ${escapeHtml(domain.name)} AI Agents · AI Agent Radar</title><script type="application/ld+json">${schema}</script></head><body><header class="site-header"><a class="brand" href="/">AI Agent Radar</a><nav class="site-nav"><a href="/">Home</a><a href="/opportunities">Opportunities</a><a href="/patterns">Patterns</a><a href="/workspace">Workspace</a><a href="/weekly">Weekly Radar</a><a href="/about">About</a></nav></header><main class="page-shell report-shell"><a class="back-link" href="/">← All agents</a><section class="report-hero"><span class="eyebrow">PROFESSIONAL DOMAIN REPORT</span><h1>${escapeHtml(domain.name)} <span>AI Agents</span></h1><p>${escapeHtml(domain.description)} Every project is open source and ranked from public GitHub adoption, maintenance, quality, relevance and demand evidence.</p></section><section class="weekly-brief"><span class="analysis-label">CURRENT COVERAGE</span><h2>${agents.length} curated project${agents.length === 1 ? '' : 's'}</h2><p>Coverage changes as the GitHub scanner discovers and re-evaluates projects. Rankings are discovery signals, not paid placement or hands-on product reviews.</p></section><section class="weekly-list">${agents.map(card).join('') || '<section class="workspace-empty"><h2>Coverage is developing</h2><p>No project currently clears the Radar threshold for this domain. The scanner will continue searching.</p></section>'}</section><section class="method-card report-method"><span class="analysis-label">PERSONAL RADAR</span><h2>Keep watching ${escapeHtml(domain.name)} needs</h2><p>Your free workspace remembers this field, highlights newly qualified needs on return, and turns any signal into a seven-day validation sprint.</p><div class="hero-actions"><a class="button button-primary" href="/login?domain=${encodeURIComponent(domain.name)}&amp;next=%2Fworkspace">Track ${escapeHtml(domain.name)} needs free</a><a class="button button-secondary" href="/opportunities?domain=${encodeURIComponent(domain.name)}">Explore demand signals</a></div></section></main><footer class="site-footer"><p>AI Agent Radar · Free, independent open-source intelligence</p><nav class="footer-links"><a href="/methodology">Methodology</a><span>·</span><a href="/privacy-policy">Privacy Policy</a><span>·</span><a href="/contact">Contact</a></nav></footer></body></html>`;
    res.setHeader('Content-Type', 'text/html; charset=utf-8');
    res.setHeader('Cache-Control', 'public, s-maxage=1800, stale-while-revalidate=3600');
    return res.status(200).send(html.replace('<meta name="robots" content="index, follow">', `<meta name="robots" content="${agents.length ? 'index, follow' : 'noindex, follow'}">`).replace('</head>', `<meta property="og:type" content="website"><meta name="twitter:card" content="summary_large_image"><meta name="twitter:title" content="Open-Source ${escapeHtml(domain.name)} AI Agents"><meta name="twitter:description" content="${escapeHtml(domain.description)}"><meta name="twitter:image" content="${SITE_URL}/og-image.png"></head>`).replace('<section class="weekly-list">', `${demandMarkup}<section class="weekly-list">`).replace('</nav></header>', '<a href="/login">Sign in</a></nav></header>').replace('</body>', '<script src="/analytics.js"></script></body>'));
  } catch (error) {
