...
function validationAction(agent, profile) {
  const issue = profile.issues.find((item) => /^\d+$/.test(String(item.id || '')));
  if (!issue) return '';
  return `<a class="button button-primary" href="/opportunity/${encodeURIComponent(issue.id)}#validation-start">Test ${escapeHtml(agent.name)}'s strongest need</a>`;
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).send(enrichedHtml.replace('</body>', '<script src="/analytics.js"></script></body>'));
  } catch (error) {
