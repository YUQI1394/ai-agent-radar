...
function automaticAnalysis(agent, agents) {
  ...
  const ordered = [...agents].sort((a, b) => Number(b.stars ?? b.votes ?? 0) - Number(a.stars ?? a.votes ?? 0));
...
<body>
  ...
  <main class="page-shell detail-shell">
    ...
    <article class="detail-card">
      ...
      <div class="detail-stats"><div><strong>${score.total}/100</strong><span>Transparent Radar Score</span></div><div><strong>★ ${Number(agent.stars ?? agent.votes ?? 0).toLocaleString()}</strong><span>GitHub stars${Number(agent.starDelta ?? agent.voteDelta ?? 0) > 0 ? ` · +${Number(agent.starDelta ?? agent.voteDelta)} last scan` : ''}</span></div><div><strong>${escapeHtml(agent.pushedAt ? new Date(agent.pushedAt).toLocaleDateString('en-US') : '—')}</strong><span>Last code update</span></div></div>
      ...
      <section class="score-breakdown"><div><span>Adoption</span><strong>${score.adoption ?? score.community}/30</strong></div><div><span>Maintenance</span><strong>${score.maintenance ?? score.freshness}/25</strong></div><div><span>Project quality</span><strong>${score.quality || 0}/20</strong></div><div><span>Agent relevance</span><strong>${score.relevance}/20</strong></div><div><span>Momentum</span><strong>${score.momentum}/5</strong></div><p>Adoption uses GitHub stars and forks; maintenance uses recent code activity; quality checks license and repository metadata; relevance uses topics and descriptions; momentum uses star growth between scans.</p></section>
      ...
      <section class="trend-observation"><span class="analysis-label">OPEN-SOURCE SIGNAL</span><h2>What the evidence suggests</h2><p>${escapeHtml(agent.name)} ranks #${analysis.rank} among ${agents.length} tracked projects using public GitHub adoption, maintenance, relevance and momentum signals.</p><small>Automatically generated from public repository metadata. It is not a paid placement, endorsement, security audit or hands-on review.</small></section>
      <div class="detail-actions"><a class="button button-primary detail-visit" href="${safeUrl(agent.githubUrl || agent.url)}" target="_blank" rel="noopener noreferrer">View source on GitHub ↗</a>${primaryPeer ? `<a class="button button-secondary detail-visit" href="/compare?agents=${slug},${encodeURIComponent(primaryPeer.slug || primaryPeer.id)}">Compare with ${escapeHtml(primaryPeer.name)}</a>` : ''}</div>
    </article>
  </main>
