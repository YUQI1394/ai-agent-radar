...
const { cleanIssueEvidence, coachingPlan, evidenceConfidence, opportunityPattern } = require('../lib/opportunity-themes');
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const professionalDomain = agent.category || category(agent);
    const coach = coachingPlan(issue, professionalDomain);
    const targetPattern = opportunityPattern(issue).name;
    const repositoryCount = new Set((latest?.agents || []).filter((currentAgent) => cleanIssueEvidence(currentAgent.evidenceIssues || []).some((candidate) => opportunityPattern(candidate).name === targetPattern)).map((currentAgent) => String(currentAgent.id || currentAgent.slug || currentAgent.name))).size || 1;
    const confidence = evidenceConfidence(issue, repositoryCount);
    ...
    const nextActions = [
      ...
      `Schedule five interviews with ${coach.audience} and record one recent example from each.`,
      coach.experiment,
      'Review the evidence and choose Build, Narrow or Stop.'
    ].map((action) => escapeHtml(String(action).slice(0, 180)));
    const coachMarkup = `<section class="professional-coach-note"><span class="analysis-label">${escapeHtml(professionalDomain.toUpperCase())} VALIDATION LENS</span><p><strong>Recruit:</strong> ${escapeHtml(coach.recruitment)}</p><p><strong>Guardrail:</strong> ${escapeHtml(coach.constraint)}</p></section><section class="coach-grid" data-coaching-theme="${escapeHtml(coach.theme.slug)}"><article data-next-action="${nextActions[0]}"><span class="coach-step">01 · ${escapeHtml(coach.pattern.name)}</span><h2>Write the problem hypothesis</h2><p>${escapeHtml(coach.hypothesis)}</p><label>Done when</label><span>You can name one user, one situation and one measurable consequence without proposing a feature.</span></article><article data-next-action="${nextActions[1]}"><span class="coach-step">02 · EVIDENCE INTERVIEW</span><h2>Interview five ${escapeHtml(coach.audience)}</h2><ul>${questions}</ul><label>Done when</label><span>At least three people independently describe the same painful workflow with recent examples.</span></article><article data-next-action="${nextActions[2]}"><span class="coach-step">03 · MINIMUM TEST</span><h2>Run the smallest experiment</h2><p>${escapeHtml(coach.experiment)}</p><label>Behavioral proof</label><span>${escapeHtml(coach.proof)}</span><label>Professional constraint</label><span>${escapeHtml(coach.constraint)}</span></article><article data-next-action="${nextActions[3]}"><span class="coach-step">04 · DECISION GATE</span><h2>Make a build decision</h2><ul><li><strong>Build:</strong> repeated pain and active commitment</li><li><strong>Narrow:</strong> pain is real but the audience or job differs</li><li><strong>Stop:</strong> weak frequency or no behavioral proof</li></ul><label>Rule</label><span>Do not let GitHub engagement replace direct validation.</span></article></section>`;
