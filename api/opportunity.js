...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).send(html.replace(/<section class="coach-grid">[\s\S]*?<\/section><section class="method-card/, `${coachMarkup}<section class="method-card`).replace('</nav></header>', '<a href="/login">Sign in</a></nav></header>').replace('</body>', '<script src="https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2.57.4/dist/umd/supabase.min.js" crossorigin="anonymous"></script><script src="/auth.js"></script><script src="/cloud-storage.js"></script><script src="/validation.js"></script></body>'));
  } catch (error) {
