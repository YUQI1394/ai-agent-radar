...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).send(html.replace('</head>', `<meta property="og:type" content="article"><meta name="twitter:card" content="summary_large_image"><meta name="twitter:title" content="${escapeHtml(issue.title)} · Opportunity Radar"><meta name="twitter:description" content="Trace the GitHub evidence and run a focused four-step validation sprint."><meta name="twitter:image" content="${SITE_URL}/og-image.png"></head>`).replace(/<section class="coach-grid">[\s\S]*?<\/section><section class="method-card/, `${coachMarkup}<section class="method-card`).replace('</nav></header>', '<a href="/login">Sign in</a></nav></header>').replace('</body>', '<script src="https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2.57.4/dist/umd/supabase.min.js" integrity="sha384-AkNSQdptcXlJ0/NBZc4qGk86cDVXcCevwoWgEKIpHOEfbvlXGLlIkimQtONt8KNf" crossorigin="anonymous"></script><script src="/auth.js"></script><script src="/cloud-storage.js"></script><script src="/validation.js"></script></body>'));
  } catch (error) {
