...
function underSourcedCurrentAgents(currentAgents = [], archivedAgents = [], minimumSources = TARGET_CATEGORY_EVIDENCE_PROJECTS, minimumEvidence = TARGET_CATEGORY_EVIDENCE) {
  const sourcesByCategory = currentAgents.reduce((counts, agent) => {
  ...
  return [...currentAgents, ...archivedAgents]
    .filter((agent) => {
      const key = String(agent.category || 'General AI');
      return (sourcesByCategory.get(key) || 0) < minimumSources || (evidenceByCategory.get(key) || 0) < minimumEvidence;
    })
    .map((agent) => ({ ...agent, status: 'current' }));
}
...
function selectIssueTargets(candidates, currentAgents = [], archivedAgents = [], limit = ISSUE_TARGETS_PER_SCAN) {
  ...
  // Give every thin professional field one current-project scan before using
  // the remaining request budget for discovery. Otherwise an eight-field
  // recovery cycle with a seven-project current budget always skips a field.
  const recoveryDomainCount = new Set(current.filter(needsRecovery).map((agent) => String(agent.category || 'General AI'))).size;
  const currentLimit = discovery.length ? Math.min(limit, Math.max(CURRENT_ISSUE_TARGETS_PER_SCAN, recoveryDomainCount)) : limit;
...
module.exports = { selectIssueTargets, underSourcedCurrentAgents };
