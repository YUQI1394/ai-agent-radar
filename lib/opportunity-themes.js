...
const PATTERNS = [
  ...
  { name: 'Tool & system connectors', theme: 'Integrations', pattern: /connector|plugin|webhook|mcp|integrat|api\b/i },
  { name: 'Setup & configuration', theme: 'Developer Experience', pattern: /install|setup|config|environment|deploy|docker/i },
  { name: 'Debugging & observability', theme: 'Agent Control', pattern: /debug|observ|trace|log|monitor|telemetry/i },
  ...
  { name: 'Human approval & safety', theme: 'Agent Control', pattern: /human|approval|permission|access.?control|guardrail|security|privacy/i },
  ...
  { name: 'Runtime & deployment environments', theme: 'Product Capability', pattern: /\bwsl\b|windows users?|dedicated cloud|local ai|locally deployed|runtime-owned/i },
  { name: 'Interface & rendering', theme: 'Product Capability', pattern: /markdown rendering|web ui|html routes?|prompt is not always returned/i },
  ...
  { name: 'Multi-user & team routing', theme: 'Agent Control', pattern: /multi-user|team members?|several agents|nearest answers/i },
...
const MAINTENANCE_NOISE = /dependenc(?:y|ies) (?:dashboard|update|refresh|bump)|daily security sweep|release checklist|roadmap tracking|ci red|build status|build artifacts?|automated update|test matrix|canary|flaky test|tests? (?:fail|failing)|verification.*manifest|security policy (?:is )?(?:not )?(?:fully )?set up|security[- ]scanning baseline|valid license of this product|gsoc.*community|already[- ]fixed|duplicate|wontfix|invalid/i;
const INTERNAL_WORK = /^(?:chore|test|ci|build|tracking|intake|publish|triage|plan(?:-\d+)?)\b/i;
...
const VAGUE_REPORT = /^(?:not parsed|no results whatsoever|denied request(?:,.*)?|ctrl\+v (?:doesn'?t|dosen'?t|does not) work|import .{0,30} issue)$/i;
...
function isUsefulDemandSignal(issue = {}) {
  ...
  if (title.length < 12 || MAINTENANCE_NOISE.test(`${title} ${labels}`)) return false;
...
function coachingPlan(issue) {
  ...
  return { theme, pattern, ...(PATTERN_COACHING[pattern.name] || COACHING[theme.slug]) };
}
