...
const MAINTENANCE_NOISE = /dependenc(?:y|ies) (?:dashboard|update|refresh|bump)|daily security sweep|release checklist|roadmap tracking|tag-driven workflow.{0,40}releases?|\b(?:five-stack )?integration tracker\b|clean-room evaluation|current conformance (?:manifest|corpus)|production soak[- ]test|before default-on|architecture validation.{0,100}(?:doc.?drift|invariant gaps?)|doc.?drift findings|non-critical invariant gaps?|ci red|build status|build artifacts?|automated update|test matrix|canary|flaky test|tests? (?:fail|failing)|verification.*manifest|security policy (?:is )?(?:not )?(?:fully )?set up|security[- ]scanning baseline|valid license of this product|gsoc.*community|already[- ]fixed|duplicate|wontfix|invalid/i;
...
const PURE_CONFIGURATION_QUESTION = /^(?:how (?:do|can|to)\b|where (?:do|can|to)\b).{0,120}\b(?:api key|token|credential|secret|\.env|environment variable)\b/i;
...
const DEFECT_SIGNAL = /\b(?:bug|errors?|fails?|failure|not working|unable to|cannot|can't|crash|broken|doesn't work)\b|一直报错|链接失败|无法|不能/i;
...
function isUsefulDemandSignal(issue = {}) {
  ...
  if (DEFECT_SIGNAL.test(`${title} ${labels}`) && reactions === 0 && comments <= 4) return false;
  if (REPOSITORY_DEFECT.test(title) && reactions < 2) return false;
  if (PURE_CONFIGURATION_QUESTION.test(withoutAllowedPrefix) && reactions < 2) return false;
...
function patternSlug(pattern = '') {
  const name = typeof pattern === 'string' ? pattern : pattern.slug || pattern.name || '';
...
function evidenceConfidence(issue = {}, repositoryCount = 1) {
  const repositories = Math.max(1, Number(repositoryCount || 1));
  const comments = Math.max(0, Number(issue.comments || 0));
  const reactions = Math.max(0, Number(issue.reactions || 0));
  if (repositories >= 2) return {
    level: 'repeated',
    label: `REPEATED ACROSS ${repositories} PROJECTS`,
    description: `Related friction appears in ${repositories} independent repositories. This is stronger than one backlog item, but still requires direct user validation.`
  };
  if (reactions >= 3 || comments >= 8) return {
    level: 'supported',
    label: 'STRONG ISSUE SUPPORT',
    description: 'This Issue has meaningful public discussion or positive reactions, but evidence still comes from one repository.'
  };
  if (reactions >= 1 || comments >= 5) return {
    level: 'supported',
    label: 'SUPPORTED ISSUE',
    description: 'This Issue has some independent public support. Treat it as a focused validation lead, not proof of a market.'
  };
  return {
    level: 'early',
    label: 'EARLY INTERVIEW LEAD',
    description: 'Public support is still limited. Use this signal to recruit interviews before considering a product decision.'
  };
}

module.exports = { PATTERNS, THEMES, cleanIssueEvidence, coachingPlan, evidenceConfidence, evidenceEngagement, evidenceFreshness, evidenceStrength, isUsefulDemandSignal, issueExcerpt, issueFingerprint, opportunityPattern, opportunityTheme, patternSlug };
