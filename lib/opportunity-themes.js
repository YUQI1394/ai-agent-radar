...
const PATTERNS = [
  ...
  { name: 'Provider interoperability', theme: 'Integrations', pattern: /provider|model support|openai|anthropic|ollama|gemini|azure|bedrock/i },
...
function opportunityPattern(issue) {
  ...
  return PATTERNS.find((pattern) => pattern.theme === theme.name && pattern.pattern.test(text)) || { name: `${theme.name} workflows`, theme: theme.name };
}
