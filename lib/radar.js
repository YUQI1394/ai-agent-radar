...
function scoreBreakdown(agent, now = Date.now()) {
  ...
  const adoption = Math.min(40, Math.round(Math.log10(stars + 1) * 10 + Math.log10(forks + 1) * 4));
  ...
  const relevance = /agent|agentic|autonomous|multi.?agent/i.test(relevanceText) ? 25 : /copilot|assistant|workflow|automation|mcp/i.test(relevanceText) ? 19 : 10;
  const momentum = Math.min(10, Math.max(0, Math.round(Math.log2(Number(agent.starDelta || 0) + 1) * 2)));
  return { adoption, maintenance, relevance, momentum, community: adoption, freshness: maintenance, total: Math.min(100, adoption + maintenance + relevance + momentum) };
}
