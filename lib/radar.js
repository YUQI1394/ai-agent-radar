...
function scoreBreakdown(agent, now = Date.now()) {
  ...
  const adoption = Math.min(30, Math.round(Math.log10(stars + 1) * 5 + Math.log10(forks + 1) * 2.5));
  const maintenance = ageDays <= 7 ? 25 : ageDays <= 30 ? 21 : ageDays <= 90 ? 15 : ageDays <= 180 ? 9 : 3;
  ...
  return { adoption, maintenance, quality, relevance, momentum, community: adoption, freshness: maintenance, total: Math.min(100, adoption + maintenance + quality + relevance + momentum) };
}

function qualityLabel(agent) {
  const score = agent.score || scoreBreakdown(agent);
  if (score.total >= 94 && score.quality >= 17) return 'HIGH CONFIDENCE';
  if (score.total >= 82) return 'ESTABLISHED';
  if (score.maintenance >= 21 && score.total >= 65) return 'ACTIVE';
  return 'WATCH';
}
