...
async function main() {
  ...
  const searchConsoleLegacy = await fetch(`${origin}/agent/tines-3b`, { redirect: 'manual' });
  assert.equal(searchConsoleLegacy.status, 410, `Search Console legacy listing returned ${searchConsoleLegacy.status}`);
  assert.match(searchConsoleLegacy.headers.get('x-robots-tag') || '', /noindex/i, 'Search Console legacy listing is indexable');
  const retiredComparison = await fetch(`${origin}/compare?agents=posthog-desktop-3%2Cdograh-3`, { redirect: 'manual' });
