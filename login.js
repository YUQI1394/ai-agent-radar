(() => {
  ...
  introduction.insertAdjacentHTML('afterend', '<ul class="auth-benefits"><li><strong>A concrete first action</strong><span>Automatically scheduled when you start a sprint.</span></li><li><strong>Evidence, not memory</strong><span>Structure interviews by incident, workaround, cost and behavioral commitment.</span></li><li><strong>A watchlist that moves</strong><span>See demand evidence, star growth and rank changes for saved projects.</span></li><li><strong>An honest decision gate</strong><span>See whether Build is evidence-backed or still only provisional.</span></li></ul><section class="registration-personalizer" aria-labelledby="registration-personalizer-title"><div><span>PERSONALIZE YOUR START</span><label id="registration-personalizer-title" for="registration-domain">Choose your professional field</label></div><select id="registration-domain"><option>Research</option><option>Security</option><option>Finance</option><option>Marketing</option><option>Coding</option><option>Design</option><option>Productivity</option><option>Agent Infrastructure</option></select><article id="registration-recommendation" aria-live="polite"><p>Loading a current GitHub-backed need…</p></article><small>Public evidence stays free. Registration saves your sprint and syncs progress.</small></section><section class="outcome-preview" aria-labelledby="outcome-preview-title"><div class="outcome-preview-heading"><span>YOUR 7-DAY BOARD</span><strong id="outcome-preview-title">See what registration creates</strong></div><div class="outcome-preview-body"><p><span>Selected live need</span><strong id="outcome-preview-need">Loading from the current Radar…</strong><small id="outcome-preview-source"></small></p><dl><div><dt>Starting point</dt><dd>0 / 5 proof gates · decision undecided</dd></div><div><dt>First action</dt><dd id="outcome-preview-action">Frame the problem without proposing a feature</dd></div><div><dt>Finish</dt><dd class="outcome-build">EVIDENCE-BACKED BUILD, NARROW OR STOP</dd></div></dl></div></section>');
  card.querySelector('.auth-benefits').insertAdjacentHTML('beforeend', '<li><strong>A decision brief you can use</strong><span>Export the evidence, confidence and recommendation—not a data dump.</span></li>');
  ...
  const outcomeNeed = document.querySelector('#outcome-preview-need');
  const outcomeSource = document.querySelector('#outcome-preview-source');
  ...
  fetch('/api/get-agents').then(async (response) => {
    ...
    outcomeNeed.textContent = 'Current sprint preview temporarily unavailable';
    outcomeSource.textContent = 'Your workspace will load current qualified evidence after sign-in.';
  });
